...
|---------|---------------|
| `internal/schema` | Map/list/schema-block YAML formats; column attributes (nullable, notNull, default, unique, primaryKey, identity); primary keys; foreign keys; indexes; check/unique/exclude constraints; triggers; extensions; enum types; composite types; functions (security, volatility, strict); procedures (args, security, set, body, grants, `revokePublic`, `dependsOn`, multi-file merge); views; materialized views; sequences (options, defaults, `dependsOn`, multi-file merge); domains (options, defaults, `dependsOn`, multi-file merge); grants (table/function/schema, `revokePublic`); roles (options, `inRoles`, defaults, multi-file merge); RLS + policies; comments (all object types); `dependsOn`; topological sort; `LoadAndMerge` including missing-file tolerance; `qualify` helper |
