...
|---------|---------------|
| `internal/schema` | Map/list/schema-block YAML formats; column attributes (nullable, notNull, default, unique, primaryKey, identity, using); primary keys; foreign keys; indexes (unique, `using` method, `where` predicate); check/unique/exclude constraints; triggers (incl. `when` guard); extensions; enum types; composite types; functions (security, volatility, strict); procedures (args, security, set, body, grants, `revokePublic`, `dependsOn`, multi-file merge); views; materialized views; sequences (options, defaults, `dependsOn`, multi-file merge); domains (options, defaults, `dependsOn`, multi-file merge); grants (table/view/matview/function/schema/column, `revokePublic`); roles (options, `inRoles`, defaults, multi-file merge); RLS + policies; comments (all object types); partitioning (partitionBy explicit + shorthand, partitionOf range/list/hash/default bounds, qualified parents, parent-before-child topological order); `dependsOn`; topological sort (incl. cycle detection error, function-after-table ordering); `LoadAndMerge` including missing-file tolerance; `qualify` helper |
